            ...
            <div className="flex items-start space-x-3 sm:space-x-4">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {aiResponse && (
                        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-500 p-3 sm:p-4 rounded-r-xl mb-4 shadow-sm">
                            <div className="flex items-center space-x-2 mb-2 flex-wrap">
                                ...
                                <span className="text-xs sm:text-sm font-semibold text-blue-900">AI Support Response</span>
                                <span className="text-blue-300 hidden sm:inline">•</span>
                                <span className="text-xs text-blue-700 hidden sm:inline">{formatDate(new Date(aiResponse.timestamp))}</span>
                            </div>
                            <div className="text-xs sm:text-sm text-blue-900 leading-relaxed break-words">
                                {aiResponse.content}
                            </div>
                        </div>
                    )}
                            ...
                            {replies.map((reply) => (
                                <div key={reply.id} className="bg-gray-50 p-3 rounded-lg ml-4 border border-gray-100">
                                    <div className="flex items-center space-x-2 mb-1">
                                        ...
                                        <span className="text-gray-300">•</span>
                                        <span className="text-xs text-gray-500">{formatDate(new Date(reply.timestamp))}</span>
                                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-xs sm:text-sm text-gray-500">
                        <span className="bg-gray-100 px-2 py-1 sm:px-3 rounded-full text-xs self-start">{shortWallet}</span>
                        ...
                        <div className="flex items-center space-x-3 sm:space-x-4">
                            ...
                            <div className="flex items-center space-x-1">
                                ...
                                <span className="text-xs sm:text-sm">{totalTips} ETH</span>
                            </div>
                        </div>
                    </div>
                </div>
